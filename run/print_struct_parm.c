...
struct buffer
{
	unsigned int size;/*size of the userspace buffer*/
	void *mem;/*memory usually we use user ptr for it*/
};
/*allocated buffers*/
...
struct device
{
	...
	enum v4l2_buf_type type;/*types are dependent on the device we are using*/
	enum v4l2_memory memtype;
	unsigned int nbufs;
	struct buffer *buffers;/*to access the buffers inside the device we declared it here*/

	unsigned int width;
	unsigned int height;
	unsigned int bytesperline;/*Distance in bytes between the leftmost pixels in two adjacent lines*/
	unsigned int imagesize;/*applies to these as well*/

	void *pattern;/*doubts here*/
};
...
static const char *v4l2_buf_type_name(enum v4l2_buf_type type)/*return of static const char ptr?? called with pass  fmt enum*/
{
	static struct {
	...
	};/*not typedefined by typedef its declared as the array of structures just storage class additional specification*/
	/*all the 3 enums got what type of buffers we need to use */
	
	
	printf("The type of buffer used is : %d %d %d \n",names[0].type,names[1].type,names[2].type);
	...
	for (i = 0; i < ARRAY_SIZE(names); ++i) {/*#define ARRAY_SIZE(a)	(sizeof(a)/sizeof((a)[0])) as struct size is declared now*/
		if (names[i].type == type)/*format type if equals  to buffer name----i is the index*/
			return names[i].name;
	}
...
static int video_open(struct device *dev, const char *devname, int no_query)
{
	...
	if (!no_query) {
		memset(&cap, 0, sizeof cap);/*memset the cap instance/struct var*/
		...
		printf("Capability parameters\n");
		printf("Device name : %s opened:cap.card : %s cap.bus.info :(%s).\n", devname, cap.card, cap.bus_info);/*display the cap param*/
...
static void uvc_set_control(struct device *dev, unsigned int id, int value)
{
	struct v4l2_control ctrl;
	int ret;
	printf("Inside the set control function\n");
	ctrl.id = id;
	ctrl.value = value;
...
/*https://youtu.be/a9CZ1Uk3OYQ?t=24*/
static int video_get_format(struct device *dev)
{
	struct v4l2_format fmt;/*formats struct*/
	int ret;

	memset(&fmt, 0, sizeof fmt);/*set those addresses*/
	fmt.type = dev->type;
		//printf("##video_get_format### ioctl(dev->fd, VIDIOC_G_FMT, &fmt); \n");
	ret = ioctl(dev->fd, VIDIOC_G_FMT, &fmt);/*acces the device format in kernel space*/
	if (ret < 0) {
		printf("Unable to get format: %s (%d).\n", strerror(errno),
			errno);
		return ret;
	}
	/*successful in getting the format*/
	...
	printf("******************************** format****************************\n");
	printf("format structure by get format: \n");
	printf("fmt.fmt.pix.width : %d\n",fmt.fmt.pix.width);
	printf("fmt.fmt.pix.height : %d\n",fmt.fmt.pix.height);
	printf("fmt.fmt.pix.bitesperline  : %d\n",fmt.fmt.pix.bytesperline);
	...
	printf("Video format: %c%c%c%c (%08x) %ux%u\n",
		...
		fmt.fmt.pix.pixelformat,
		fmt.fmt.pix.width, fmt.fmt.pix.height);/*YUYV to display them as char and w x h*/
	return 0;
}
...
static int video_set_format(struct device *dev, unsigned int w, unsigned int h, unsigned int format)
{
	...
	printf("********************************format meta data****************************\n");
	printf("After setting the video format : \n");
	printf("image size = buffer size/length\n");
	printf("Video format set: width: %u height: %u buffer size: %u\n",
		fmt.fmt.pix.width, fmt.fmt.pix.height, fmt.fmt.pix.sizeimage);

	printf("format structure by SET format: \n");
	printf("fmt.fmt.pix.width : %d\n",fmt.fmt.pix.width);
	printf("fmt.fmt.pix.height : %d\n",fmt.fmt.pix.height);
	printf("fmt.fmt.pix.bitesperline  : %d\n",fmt.fmt.pix.bytesperline);
	printf("fmt.fmt.pix.pixelformat : %d\n",fmt.fmt.pix.pixelformat);
...
static int video_set_framerate(struct device *dev, struct v4l2_fract *time_per_frame)
{
	...
	printf("******************************** time and time/frame****************************\n");
	struct v4l2_streamparm parm;/*streaming starts*/
	int ret;

	memset(&parm, 0, sizeof parm);/*taken care of nullifying as we call ioctl and kernel writes to it*/
	parm.type = dev->type;
...
static int video_alloc_buffers(struct device *dev, int nbufs, unsigned int offset)
{
	...
	printf("From the reqbuf struct : \n");
	printf("nbufs=rb.count=%d\n",rb.count);
	printf("dev->type=rb.type=%d\n",rb.type);
	printf("dev->memtype=rb.memory=%d\n",rb.memory);
	...
	for (i = 0; i < rb.count; ++i) {
		...
		printf("in v4l2 buffer buf->m \n");
		printf("offset=sizeof image : %d\n",buf.m.offset);
	
	
	
	printf("buf.type = %d , buf.memory = %d \n",buf.type,buf.memory);
...
int main(int argc, char *argv[])
{
	...
	printf("dev->fd : %d\n",dev.fd);
	printf("dev->memtype : %d\n",dev.memtype);
	printf("dev->buf_type : %d\n",dev.type);
	printf("dev->width : %d\n",dev.width);
	printf("dev->height : %d\n",dev.height);
	printf("dev->bytesperline : %d\n",dev.bytesperline);
	printf("dev->imagesize : %d\n",dev.imagesize);
