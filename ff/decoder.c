...
extern AVCodec ff_h264_decoder;/*derived from the other file*/
...
static void streamInit(Stream *stream, u32 length) {
    printf("USER: stream init \n");
    stream->buffer = stream->pos = (u8 *)malloc(sizeof(u8) * length);
    stream->length = length;
    stream->end = stream->buffer + length;/*the ending of stream will bes last position i.e start pos+length*/
}/*initializing the stream with the length of buffer and position*/
...
u8* broadwayCreateStream(u32 length) {
	printf("USER:broad way create stream \n");
...
void broadwayPlayStream(u32 length) {
	printf("USER:broadway play stream \n");
...
void broadwayParsePlayStream(u32 length) {
	printf("USER:broadwayparse play stream \n");
    broadwayStream.length = length;
...
u32 broadwayGetMajorVersion() {
	printf("USER: broadway get major version \n");
    return 1;
}
...
u32 broadwayGetMinorVersion() {
	printf("USER: broadway get minor version \n");
    return 1;
}
